...
describe CourseReporting::AggregationsController do
  ...
  it 'export renders consolidated if allowed' do
    get :export, id: group.id, year: 2014, lk: 'bk', subsidized: 'true', categories: [1,2,3], consolidate: true
    expect(response).to be_ok
  ...
  it 'export does not render consolidated if not allowed' do
    ...
    get :export, id: groups(:be).id, year: 2014, lk: 'bk', subsidized: 'true', categories: [1,2,3], consolidate: true
    expect(response).to be_ok
