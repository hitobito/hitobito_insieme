...
#  Copyright (c) 2021-2021, insieme Schweiz. This file is part of
...
describe Vp2021::CourseReporting::Aggregation do
  ...
  context 'structure and counting courses' do
      ...
      context leistungskategorie do
        ...
        it 'sums by inputkritierien' do
          kursfachkonzepte.each do |k|
            create!(create_course(leistungskategorie, [groups(:be)], 2021, k), :freizeit_und_sport)
          end
    ...
    context 'sk' do
      ...
      it 'sums once for all kursfachkonzepte' do
        kursfachkonzepte.each do |k|
          create!(create_course('sk', [groups(:be)], 2021, k), 'freizeit_und_sport')
        end
  ...
  context 'treffpunkt-aggregation' do
    ...
    let!(:treffpunkt_a_course_record) do
      create!(create_course('tp', [groups(:fr)], 2021, 'treffpunkt'), 'weiterbildung', cr_defaults.merge({
    ...
    let!(:treffpunkt_b_course_record) do
      cr_b = create!(create_course('tp', [groups(:fr)], 2021, 'treffpunkt'), 'weiterbildung', cr_defaults.merge({
  ...
  def create_course(leistungskategorie = 'bk', group_list = [groups(:be)], year = 2021, fachkonzept = 'sport_jugend')
