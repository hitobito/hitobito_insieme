...
describe Vp2021::CostAccounting::Aggregation do
  let(:year) { 2021 }
  ...
  context 'has assumptions, it' do
    it 'does not blow up' do
      expect(subject.year).to be 2021
    end

    it 'has a used API, returning [Vp2021::CostAccounting::Aggregation::Report]' do
      expect(values).to be_an Array
      expect(values.first).to be_a Vp2021::CostAccounting::Aggregation::Report
    end
