...
describe Fp2022::CostAccounting::Aggregation do
  let(:year) { 2022 }
  ...
  context "has assumptions, it" do
    it "does not blow up" do
      expect(subject.year).to be 2022
    end

    it "has a used API, returning [Fp2022::CostAccounting::Aggregation::Report]" do
      expect(values).to be_an Array
      expect(values.first).to be_a Fp2022::CostAccounting::Aggregation::Report
    end
