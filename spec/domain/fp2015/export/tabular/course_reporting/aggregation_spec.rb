...
describe Fp2015::Export::Tabular::CourseReporting::Aggregation do
  let(:values) do
    {
      kursdauer: 2,
  ...
  %w[bk tk].each do |leistungskategorie|
    context leistungskategorie do
      before do
        ...
        create!(create_course(leistungskategorie), "freizeit_und_sport", values).tap { |c|
          c.update_column(:inputkriterien, :b)
        }
        create!(create_course(leistungskategorie), "weiterbildung", values).tap { |c|
          c.update_column(:inputkriterien, :c)
        }
      end
      ...
      it "contains correct row headers, values and sums" do
        ...
        expect(rows[4]).to eq ["davon Personen mit Behinderung", 1, 1, 2, nil, 1, 1, 1, nil, 1, 4]
        expect(rows[5]).to eq ["davon Angehörige", 2, 2, 4, nil, 2, 2, 2, nil, 2, 8]
        expect(rows[6]).to eq ["davon nicht Bezugsberechtigte", nil, nil, 0, nil, nil, 0, nil, nil,
          0, 0]
        expect(rows[7]).to eq ["Absenztage", 1.5, 1.5, 3.00, 0.00, 1.5, 1.5, 1.5, 0.00, 1.5, 6.00]
        expect(rows[8]).to eq ["davon Absenztage Personen mit Behinderung", 0.50, 0.50, 1.00, nil,
          0.50, 0.50, 0.50, nil, 0.50, 2.00]
        expect(rows[9]).to eq ["davon Absenztage Angehörige", 1.00, 1.00, 2.00, nil, 1.00, 1.00,
          1.00, nil, 1.00, 4.00]
        expect(rows[10]).to eq ["davon Absenztage nicht Bezugsberechtigte", nil, nil, 0.00, nil,
          nil, 0.0, nil, nil, 0.0, 0.00]
        expect(rows[11]).to eq ["Effektive TeilnehmerInnentage", 4.5, 4.5, 9.0, 0.0, 4.5, 4.5, 4.5,
          0.00, 4.5, 18.0]
        expect(rows[12]).to eq ["davon TeilnehmerInnentage Personen mit Behinderung", 1.5, 1.5,
          3.00, nil, 1.5, 1.5, 1.5, nil, 1.5, 6.00]
        expect(rows[13]).to eq ["davon TeilnehmerInnentage Angehörige", 3.00, 3.00, 6.00, nil,
          3.00, 3.00, 3.00, nil, 3.00, 12.00]
        expect(rows[14]).to eq ["davon TeilnehmerInnentage nicht Bezugsberechtigte", 0.00, 0.00,
          0.00, nil, 0.00, 0.00, 0.00, nil, 0.00, 0.00]
        ...
        expect(rows[17]).to eq ["davon Fachpersonen hochqualifiziert", 2, 2, 4, nil, 2, 2, 2, nil,
          2, 8]
        expect(rows[18]).to eq ["davon Hilfspersonal ohne Honorar", 3, 3, 6, nil, 3, 3, 3, nil, 3,
          12]
        expect(rows[19]).to eq ["davon Hilfspersonal mit Honorar", 4, 4, 8, nil, 4, 4, 4, nil, 4,
          16]
        expect(rows[20]).to eq ["Anzahl Küchenpersonal", 5, 5, 10, nil, 5, 5, 5, nil, 5, 20]
        expect(rows[21]).to eq ["Gesamtaufwand direkte Kosten", 60.00, 60.00, 120.00, nil, 60.00,
          60.00, 60.00, nil, 60.00, 240.00]
        expect(rows[22]).to eq ["davon Honorare", 10.00, 10.00, 20.00, nil, 10.00, 10.00, 10.00,
          nil, 10.00, 40.00]
        expect(rows[23]).to eq ["davon Unterkunft/Raumaufwand", 20.00, 20.00, 40.00, nil, 20.00,
          20.00, 20.00, nil, 20.00, 80.00]
        expect(rows[24]).to eq ["davon übriger Aufwand inkl. Verpflegung", 30.00, 30.00, 60.00,
          nil, 30.00, 30.00, 30.00, nil, 30.00, 120.00]
        expect(rows[25][0]).to eq "Durchschnittliche direkte Kosten pro TeilnehmerInnentag"
        expect(rows[25][1..].collect(&:to_i)).to eq [13, 13, 13, 0, 13, 13, 13, 0, 13, 13]
        expect(rows[26]).to eq ["Vollkosten", 70.00, 70.00, 140.00, 0.00, 70.00, 70.00, 70.00,
          0.00, 70.00, 280.00]
        expect(rows[27][0]).to eq "Durchschnittliche Vollkosten pro TeilnehmerInnentag"
        expect(rows[27][1..].collect(&:to_i)).to eq [15, 15, 15, 0, 15, 15, 15, 0, 15, 15]
        expect(rows[28]).to eq ["Beiträge Teilnehmende", 10.00, 10.00, 20.00, nil, 10.00, 10.00,
          10.00, nil, 10.00, 40.00]
        expect(rows[29]).to eq ["Betreuungschlüssel (Teilnehmende / Betreuende)", 0.10, 0.10, 0.10,
          0, 0.10, 0.10, 0.10, 0, 0.10, 0.10]
        expect(rows[30]).to eq ["Anzahl Kurse mit spezieller Unterkunft", 1, 1, 2, 0, 1, 1, 1, 0,
          1, 4]
      end
    end
  end
  ...
  context "sk" do
    ...
    it "contains correct row headers, values and sums" do
      ...
      expect(rows[2]).to eq ["Anzahl Kursstunden", BigDecimal("4.0"), BigDecimal("4.0"),
        BigDecimal("8.0")]
      expect(rows[3]).to eq ["Anzahl effektive TeilnehmerInnen", 6, 6, 12]
      expect(rows[4]).to eq ["davon Personen mit Behinderung", 2, 2, 4]
      ...
      expect(rows[8]).to eq ["davon Absenzstunden Personen mit Behinderung", 4.00, 4.00, 8.00]
      ...
      expect(rows[25][1..].collect(&:to_i)).to eq [20, 20, 20]
      ...
      expect(rows[27][1..].collect(&:to_i)).to eq [23, 23, 23]
