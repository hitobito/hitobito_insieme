...
describe "CostAccounting::Aggregation" do
  let(:year) { 2016 }
  let(:aggregation) { fp_class("CostAccounting::Aggregation").new(year) }

  let(:table_be) { fp_class("CostAccounting::Table").new(groups(:be), year) }
  let(:table_fr) { fp_class("CostAccounting::Table").new(groups(:fr), year) }
  let(:table_se) { fp_class("CostAccounting::Table").new(groups(:seeland), year) }
