...
describe Export::Csv::Event::GeneralCostAllocation do
  ...
  context 'with entry' do
    ...
    it 'contains all data' do
      data = export.each { |row| row.collect! { |v| v.is_a?(BigDecimal) ? v.to_f.round(2) : v } }
      expect(data[1]).to eq(['Blockkurse', 11000.0, nil, 0.0])
      expect(data[2]).to eq(['Tageskurse', nil, 5000.0, nil])
      expect(data[3]).to eq(['Semester-/Jahreskurse', 3000.0, 1000.0, 0.33])
    end
  end
