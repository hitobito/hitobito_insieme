...
describe Fp2026::CostAccounting::Aggregation do
  let(:year) { 2026 }
  ...
  context "has assumptions, it" do
    it "does not blow up" do
      expect(subject.year).to be 2026
    end

    it "has a used API, returning [Fp2026::CostAccounting::Aggregation::Report]" do
      expect(values).to be_an Array
      expect(values.first).to be_a Fp2026::CostAccounting::Aggregation::Report
    end
