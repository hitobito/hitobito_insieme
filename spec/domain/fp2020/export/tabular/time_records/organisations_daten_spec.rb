...
describe Fp2020::Export::Tabular::TimeRecords::OrganisationsDaten do
  let(:year) { 2020 }
  subject { described_class.new(data) }

  let(:teiler) { (1900 * 130).to_d }
  let(:data) do
    od = Fp2020::TimeRecord::OrganisationsDaten.new(year)
    verein_ids = od.vereine.map(&:id)

    od.instance_variable_set('@data_for', {
      verein_ids[0] => Fp2020::TimeRecord::OrganisationsDaten::Data.new(10, 5, 20, 8, 4, 123_500, 61_750, teiler),
      verein_ids[1] => Fp2020::TimeRecord::OrganisationsDaten::Data.new(*Array.new(7) {0}, teiler),
      verein_ids[2] => Fp2020::TimeRecord::OrganisationsDaten::Data.new(10, 5, 20, 7, 4, 0, 0, teiler),
      verein_ids[3] => Fp2020::TimeRecord::OrganisationsDaten::Data.new(*Array.new(7) {0}, teiler)
    })
